...
import Tooltip from 'react-tooltip-lite'; // Si decides usar una biblioteca como react-tooltip-lite
  ...
  const reservarModulos = () => {
    ...
    setModulosSeleccionados([]);
      ...
      const actualizarExamenesConfirmados = (examenes) => {
        ...
        //correo
        enviarCorreoReserva(selectedExam,salaSeleccionada, {fecha:"fecha"},{email:"mariaignacia.cobo@gmail.com",nombre:'prueba'}, selectedExam.docente);
      };
     ...
     const handleSearch = () => {
      ...
      const resultadoOrdenado = resultadoBusquedaExam.sort((a, b) => a.reservado - b.reservado);

      setSearchResultExam(resultadoOrdenado);
    };
 ...
 */
  ...
  const enviarCorreoReserva = (examen,sala,modulo, alumno, docente) => {
    const emailData = {
        to: [alumno.email, docente.email],
        subject: 'Confirmación de Reserva de Examen',
        text: `
            Estimados,

            Se ha realizado la programación del siguiente examen:

            - Evento: ${examen.evento}
            - Sección: ${examen.seccion}
            ...
            Saludos,
            Equipo de Planificación de Exámenes
        `,
    };
    ...
    return (
      <>
        {initialDayStructure.map((_, indexModulo) => (
          <tr key={indexModulo}>
            <td>{indexModulo + 1}</td>
            <td className='colHorario'>{rangosHorarios[indexModulo]}</td>
